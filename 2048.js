  ...
  actuate(grid, metadata) {
    ...
    window.requestAnimationFrame(function () {
      ...
      if (self.score > self.bestScore) {
		...
		var highscoreRef = database.ref('highscores/' + userId);

            // Speichere den Highscore in der Datenbank
            highscoreRef.set({
                score: score
...
class Tile {
  ...
  updatePosition(position) {
    ...
    this.y = position.y;
        ...
        // Lese die Rangliste aus der Datenbank und zeige sie an
        function displayHighscores() {
            var highscoresRef = database.ref('highscores').orderByChild('score').limitToLast(10);

            highscoresRef.on('value', function(snapshot) {
                // Lösche vorherige Einträge
                document.getElementById('highscores').innerHTML = '';

                // Iteriere durch die Highscores und füge sie der Liste hinzu
                snapshot.forEach(function(childSnapshot) {
                    var userData = childSnapshot.val();
                    var userId = childSnapshot.key;
                    var listItem = document.createElement('li');
                    listItem.textContent = 'UserID: ' + userId + ', Score: ' + userData.score;
                    document.getElementById('highscores').appendChild(listItem);
                });
            });
        }

        // Rufe die Funktion auf, um die Rangliste anzuzeigen
        displayHighscores();
